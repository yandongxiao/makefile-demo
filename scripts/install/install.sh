...
EOF
  ...
  # 3. 离线安装 SpaceVim
  cd /tmp
  wget https://yandongxiao-1254073058.cos.ap-beijing.myqcloud.com/tools/marmotVim.tar.gz -O marmotVim.tar.gz
  ...
  # 4. Go 工具安装
  cd /tmp
  wget https://yandongxiao-1254073058.cos.ap-beijing.myqcloud.com/tools/gotools-for-spacevim.tgz
  mkdir -p $GOPATH/bin
...
function iam::install::prepare_iam()
{
  ...
  mkdir -p $WORKSPACE/golang/src/github.com/yandongxiao && cd $WORKSPACE/golang/src/github.com/yandongxiao && rm -rf iam
  git clone $(iam::install::obtain_branch_flag) --depth=1 https://github.com/yandongxiao/iam

  # NOTICE: 因为切换编译路径，所以这里要重新赋值 IAM_ROOT 和 LOCAL_OUTPUT_ROOT
  IAM_ROOT=$WORKSPACE/golang/src/github.com/yandongxiao/iam
...
export IAM_ROOT="$GOWORK/github.com/yandongxiao/iam"
alias mm="cd $GOWORK/github.com/yandongxiao"
alias i="cd $GOWORK/github.com/yandongxiao/iam"
EOF
  fi
...
EOF
  ...
  # 6. 配置hosts
  if ! egrep -q 'iam.*yandongxiao.com' /etc/hosts;then
    echo ${LINUX_PASSWORD} | sudo -S bash -c "cat << 'EOF' >> /etc/hosts
127.0.0.1 iam.api.yandongxiao.com
127.0.0.1 iam.authz.yandongxiao.com
EOF"
  fi
...
EOF
  ...
  # 3. 删除配置hosts
  echo ${LINUX_PASSWORD} | sudo -S sed -i '/iam.api.yandongxiao.com/d' /etc/hosts
  echo ${LINUX_PASSWORD} | sudo -S sed -i '/iam.authz.yandongxiao.com/d' /etc/hosts
