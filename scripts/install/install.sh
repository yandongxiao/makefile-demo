...
function iam::install::vim_ide()
{
  ...
  # 2. Go 工具安装
  wget -P /tmp/ https://yandongxiao-1254073058.cos.ap-beijing.myqcloud.com/tools/gotools-for-vim.tgz && {
    mkdir -p $GOPATH/bin
...
{
  rm -rf $WORKSPACE/golang/src/github.com/yandongxiao/iam # clean up

  # 1. 下载 iam 项目代码，先强制删除 iam 目录，确保 iam 源码都是最新的指定版本
  mkdir -p $WORKSPACE/golang/src/github.com/yandongxiao && cd $WORKSPACE/golang/src/github.com/yandongxiao
  git clone -b $(iam::install::obtain_branch_flag) --depth=1 https://github.com/yandongxiao/iam
  go work use ./iam
...
export IAM_ROOT="$GOSRC/github.com/yandongxiao/iam"
alias mm="cd $GOSRC/github.com/yandongxiao"
alias i="cd $GOSRC/github.com/yandongxiao/iam"
EOF
  fi
